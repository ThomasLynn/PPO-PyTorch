...
class PPO:
    def __init__(self, state_dim, action_dim, n_latent_var, lr, betas, gamma, K_epochs, eps_clip, beta, batches):
        self.lr = lr
        self.betas = betas
        self.gamma = gamma
    ...
    def get_batch(self, array, batch_number):
        to_return = array[(len(array)//self.batches)*batch_number:(len(array)//self.batches)*(batch_number+1)]
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            for i in range(self.batches):
                ...
                logprobs, state_values, dist_entropy = self.policy.evaluate(self.get_batch(old_states,i), self.get_batch(old_actions,i))
                    
                # Finding the ratio (pi_theta / pi_theta__old):
                ratios = torch.exp(logprobs - self.get_batch(old_logprobs,i).detach())
                    
                # Finding Surrogate Loss:
                advantages = self.get_batch(rewards,i) - state_values.detach()
                surr1 = ratios * advantages
                surr2 = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
                loss = -torch.min(surr1, surr2) + 0.5*self.MseLoss(state_values, self.get_batch(rewards,i)) - self.beta*dist_entropy
...
def main():
    ...
    ppo = PPO(state_dim, action_dim, n_latent_var, lr, betas, gamma, K_epochs, eps_clip, beta, batches)
    print(lr,betas)
